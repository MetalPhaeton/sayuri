...
namespace Misaki {
  ...
  int max_moves = 0;
  int max_level = 0;
  MoveList* CreateScore();
  void ChessBoard::Test() {
    //TestCase1();  // Pondering。
    //TestCase2();  // 2回思考する。
    TestCase3();  // コンピュータ同士の対戦。
  }
  // Pondering実験。
  void ChessBoard::TestCase1() {
    // 手を作る。
    MoveList* move_list = CreateScore();
    for (int index = 0; index < move_list->GetSize(); index++) {
      TakeMove((*move_list)[index]);
    }

    TranspositionTable* table = TranspositionTable::New();
    double searching_time = 10.0;
    double pondering_time = 30.0;
    EvalWeights weights;
    ...
    // 使用前。
    TakeMove(move);
    TranspositionTable* null_table =
    TranspositionTable::New();
    Start();
    best_move = GetBestMove(searching_time, *null_table, weights);
    Stop();
    TakeMove(best_move);
    std::cout << "before: " << GetTime() << std::endl;
    delete null_table;
    std::cout << *this;
    ...
    // Pondering。
    int pondering_depth = 4;
    StartPondering(pondering_depth, *table, weights);
    ...
    TakeMove(move);
    Start();
    best_move = GetBestMove(searching_time, *table, weights);
    Stop();
    TakeMove(best_move);
    std::cout << "after: " << GetTime() << std::endl;
    std::cout << *this;
  }
  // 2回思考する。
  void ChessBoard::TestCase2() {
    // 手を作る。
    MoveList* move_list = CreateScore();
    for (int index = 0; index < move_list->GetSize(); index++) {
      TakeMove((*move_list)[index]);
    ...
    // 1回目。
    TranspositionTable* table1 = TranspositionTable::New();
    std::cout << "<First Thinking>" << std::endl;
    Start();
    Move best_move = GetBestMove(searching_time, *table1, weights);
    Stop();
    TakeMove(best_move);
    ...
    // 2回目。
    TranspositionTable* table2 = TranspositionTable::New();
    std::cout << "<Second Thinking>" << std::endl;
    Start();
    best_move = GetBestMove(searching_time, *table2, weights);
    Stop();
    TakeMove(best_move);
    std::cout << "time: " << GetTime() << std::endl;
    std::cout << "nodes: " << nodes << std::endl;
  ...
  void ChessBoard::TestCase3() {
    ...
    int num_plys = 0;
    int high_max_level = 0;
    int low_max_level = INFINITE;
    int total_max_level = 0;
    std::cout << "Now Fighting" << std::endl;
    while (!IsGameOver()) {
      max_level = 0;
      ...
      Stop();
      delete table;
      could_move = TakeMove(best_move);
      time = GetTime();
      total_time += time;
      num_plys++;
      if (max_level > high_max_level) high_max_level = max_level;
      if (max_level < low_max_level) low_max_level = max_level;
      total_max_level += max_level;
      std::cout << "ply: " << num_plys << std::endl;
    ...
    std::cout << "high max level: " << high_max_level << std::endl;
    std::cout << "low max level: " << low_max_level << std::endl;
    std::cout << "average max level: "
    << static_cast<double>(total_max_level) / static_cast<double>(num_plys)
    << std::endl;
  }
  MoveList* CreateScore() {
  ...
  void ChessBoard::DeleteBoard() {
    for (int square = 0; square < NUM_SQUARES; square++) {
      PutPiece(static_cast<square_t>(square), EMPTY);
    }
  }
  int ChessBoard::ExamGenMove(int level) {
    int error_count = 0;
    ...
    while (ptr >= tree_ptr_[level]) {
      move = ptr->move_;
      MakeMove(move);
      king_square = ChessUtil::GetSquare(position_[side][KING]);
      if ((king_square < A1) || king_square > H8) {
        error_count++;
      } else if (IsAttacked(king_square, enemy_side)) {
        error_count++;
      }
      UnmakeMove(move);
      ptr--;
  ...
  bool ChessBoard::IsGameOver() {
    if (IsCheckmated()) return true;
    if (IsStalemated()) return true;
    if (history_[current_game_]->ply_100_ >= 100) return true;
    if (history_[current_game_]->repetition_ >= 3) return true;
    side_t side = to_move_;
    side_t enemy_side = static_cast<side_t>(side ^ 0x3);
    if (!IsEnoughPieces(side) && !IsEnoughPieces(enemy_side)) return true;
    return false;
  }
